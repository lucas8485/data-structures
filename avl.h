...
class avl final {
	...
	private:
		 ...
		 *	要求比较器具有良好实现。 
		 */
		bool _equals(const T& _lhs, const T& _rhs) const {
			// 实现依据：
			// 若p不小于q且q不小于p，则p=q。
			return (!_comparator(_lhs, _rhs)) && (!_comparator(_rhs, _lhs));
		}
